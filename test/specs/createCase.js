...
describe("Tests Create New Case Functionality", () => {
  ...
  it("Negative Test: Input unrequired fields, then save", async () => {
    await browser.refresh();
    await Case.billingToggle.click();
    await Case.fixedFeeInput.setValue(123456789);
    await CaseProcedures.assignUser();
  ...
  it("Positive Test: Checks for Retained By dropdown election", async () => {
    await Case.retainedBy.waitForExist({ timeout: 30000 });
    await Case.retainedBy.click();
    await Case.retainedBy.waitForDisplayed({ timeout: 5000 });
  ...
  it("Negative Test: Check for Adding all Affiliated users and removing", async () => {
  ...
  it("Positive Test: Assign All users, then remove", async () => {
